/**
 * Small shared helpers for the Tapfiliate adapter. Kept out of `adapter.ts` so
 * the operation methods stay readable. Reference: `src/networks/rewardful/internal.ts`.
 */
...
/** Build a `config_error` NetworkError for the Tapfiliate adapter. */
...
/**
 * Require an `AdapterCallContext` on advertiser-side operations. A Tapfiliate
 * API key scopes a single merchant account, so `networkBrandId` is currently
...
export function requireCtx(operation: string, ctx?: AdapterCallContext): AdapterCallContext {
  if (!ctx || !ctx.networkBrandId) {
    throw configErrorFor(
      operation,
      `Tapfiliate ${operation} requires a brand context (networkBrandId).`,
      {
        hint:
