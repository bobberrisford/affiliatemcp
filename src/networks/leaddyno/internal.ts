/**
 * Small shared helpers for the LeadDyno adapter. Kept out of `adapter.ts` so
...
/** Build a `config_error` NetworkError for the LeadDyno adapter. */
...
/**
 * Require an `AdapterCallContext` on advertiser-side operations. A LeadDyno
 * private key scopes a single merchant account, so `networkBrandId` is currently
...
export function requireCtx(operation: string, ctx?: AdapterCallContext): AdapterCallContext {
  if (!ctx || !ctx.networkBrandId) {
    throw configErrorFor(
      operation,
      `LeadDyno ${operation} requires a brand context (networkBrandId).`,
      {
        hint:
