...
### 3. Terms are surfaced, then accepted under one informed confirmation
...
surfaces each programme's terms to the operator as part of the per-batch
summary, and the single per-batch human confirmation is the operator's informed
acceptance of those terms. Acceptance the operator has seen and confirmed
satisfies the floor; the agent never ticks a terms box the operator has not been
shown. Where a programme's terms cannot be retrieved or displayed for review,
the skill stops on that programme and hands back rather than confirming
unseen terms.
...
subset to apply to, surfaces each programme's terms, and is gated by a single
per-batch human confirmation: the operator reviews the proposed set of
applications and the terms, and confirms once for the batch. That single
confirmation satisfies the Tier 3 human sign-off and the floor's informed-terms
requirement for the batch. The per-submit safety floor still applies on every
individual submission: the consumer respects the constraint floor on each row,
stops on a non-joinable programme, and stops and hands back on any login or MFA
challenge.
