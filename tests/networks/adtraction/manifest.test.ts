/**
 * Validate the shipped Adtraction network.json against the canonical schema.
 *
...
function loadManifest(): Record<string, unknown> {
  return JSON.parse(
    readFileSync(
      path.join(process.cwd(), 'src', 'networks', 'adtraction', 'network.json'),
      'utf8',
    ),
  );
}

describe('Adtraction network.json', () => {
  ...
  it('has the required Adtraction-specific fields set correctly', () => {
    const raw = loadManifest();
    expect(raw.slug).toBe('adtraction');
    expect(raw.name).toBe('Adtraction');
    expect(raw.base_url).toBe('https://api.adtraction.com');
    ...
    expect(raw.docs_url).toBe('https://adtractionv3.docs.apiary.io/');
    expect(raw.env_vars).toContain('ADTRACTION_API_TOKEN');
  });

  it('lists the mandatory "built from public docs" limitation as the FIRST entry', () => {
    const raw = loadManifest();
