/**
 * Validate the shipped Tradedoubler network.json against the canonical schema.
 *
...
describe('Tradedoubler network.json', () => {
  it('conforms to the canonical schema', () => {
    const raw = JSON.parse(
      readFileSync(
        path.join(process.cwd(), 'src', 'networks', 'tradedoubler', 'network.json'),
        'utf8',
      ),
    );
