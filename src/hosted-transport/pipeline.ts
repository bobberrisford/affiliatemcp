/**
 * The hosted per-request tool-call pipeline, shared by BOTH transport shapes:
 * the legacy sessionful Streamable HTTP path (`mcp-server.ts`) and the
 * MCP 2026-07-28 stateless path (`stateless-handler.ts` via
 * `stateless-surface.ts`). Extracted verbatim from `mcp-server.ts`'s
 * `CallToolRequestSchema` handler (decision
 * `docs/decisions/2026-07-29-mcp-2026-07-28-early-adoption.md`): the
 * behaviour is the legacy handler's, unchanged; only the SDK wrapper moved.
 *
 * Gate ordering, cheapest-refusal-first: tool lookup -> brand-data
 * entitlement gate -> hosted billing-tier gate (one HTTP call to the hosted
 * Worker) -> free-tier meter (one HTTP call, `free` tier only, decision
 * 2026-07-18) -> per-tier rate limit (no HTTP call) -> Solo network cap (one
 * HTTP call, vault list) -> vault-credential overlay (one HTTP call) ->
 * adapter dispatch. See `mcp-server.ts`'s file header for the full rationale.
 */
...
// Idempotent (module evaluation is cached); imported here so this module never
// silently depends on the caller's import ordering.
...
export interface HostedPipelineDeps {
  config: HostedTransportConfig;
  limiters: HostedTierRateLimiters;
}

/** The MCP tool-result shape both transports return: `content` blocks plus the
 * optional `isError` marker. Kept structural (not an SDK type import) so the
 * stateless path never depends on SDK request-handler typing; the index
 * signature is what lets one shape satisfy both the SDK's `ServerResult`
 * union (legacy path) and the stateless handler's plain-record results. */
...
/** Build the shared hosted tool registry once per process. */
export function buildHostedToolRegistry(): { tools: ToolDefinition[]; toolMap: Map<string, ToolDefinition> } {
  const tools = generateAllTools();
  return { tools, toolMap: new Map(tools.map((t) => [t.name, t])) };
}

/**
 * Run one hosted tool call through the full gate sequence and the adapter.
 * Extracted from `mcp-server.ts` verbatim; both transports call this with the
 * per-request identity already established in `call-context.ts`.
 */
export async function executeHostedToolCall(
  deps: HostedPipelineDeps,
