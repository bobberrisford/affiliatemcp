/**
 * Small shared helpers for the Refersion adapter. Kept out of `adapter.ts` so
 * the operation methods stay readable. Mirrors `src/networks/rewardful/internal.ts`.
 */
...
/** Build a `config_error` NetworkError for the Refersion adapter. */
...
/**
 * Require an `AdapterCallContext` on advertiser-side operations. A Refersion API
 * key pair scopes a single merchant account, so `networkBrandId` is currently
...
export function requireCtx(operation: string, ctx?: AdapterCallContext): AdapterCallContext {
  if (!ctx || !ctx.networkBrandId) {
    throw configErrorFor(
      operation,
      `Refersion ${operation} requires a brand context (networkBrandId).`,
      {
        hint:
