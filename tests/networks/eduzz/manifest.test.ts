/**
 * Validate the shipped Eduzz network.json against the canonical schema.
 *
...
function loadManifest(): Record<string, unknown> {
  return JSON.parse(
    readFileSync(path.join(process.cwd(), 'src', 'networks', 'eduzz', 'network.json'), 'utf8'),
  );
}

describe('Eduzz network.json', () => {
  ...
  it('has the required Eduzz-specific fields set correctly', () => {
    const raw = loadManifest();
    expect(raw.slug).toBe('eduzz');
    expect(raw.name).toBe('Eduzz');
    expect(raw.base_url).toBe('https://api2.eduzz.com');
    ...
    expect(raw.docs_url).toBe('https://developers.eduzz.com/docs/api');
  });

  it('declares exactly the three credential env vars, each upper-snake-case', () => {
    const raw = loadManifest();
    const envVars = raw.env_vars as string[];
    expect(envVars).toEqual(['EDUZZ_EMAIL', 'EDUZZ_PUBLIC_KEY', 'EDUZZ_API_KEY']);
