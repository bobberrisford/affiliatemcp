...
a publisher action. Skip any `side === 'advertiser'` entries and say so.
...
If a transaction has no `dateApproved`, fall back to its `ageDays` field (which
the adapter anchors on `dateConverted`) and flag that the age is approximate.

## Step 3 — cheap pre-check (optional but preferred)

For each network slug `s`, call
`affiliate_<s>_get_earnings_summary({ from, to })` over a window wide enough to
cover the threshold (e.g. the last 365 days). Read `oldestUnpaidAgeDays`:

- If it is absent or below the threshold, the network has nothing to chase —
  skip the transaction pull and note "nothing past threshold".
- If it meets or exceeds the threshold, proceed to step 4 for that network.
...
`minAgeDays` is a coarse server-side filter computed against `dateConverted`, so
it is a pre-filter, not the final answer. After the call, keep only the
transactions where:

- `status === "approved"`, and
- `datePaid` is absent, and
- days between `dateApproved` (or `dateConverted` fallback) and today `>=` the
  threshold.

Paginate with the returned `cursor` until exhausted. If a call fails, surface
the verbatim error (network, operation, message, httpStatus) and continue with
the remaining networks; flag the gap at the top of the output. Never silently
drop a network.
...
2. **Recipient** — leave a `[network publisher support / finance contact]`
   placeholder; you do not hold contact data. Point the user at the network's
   known support channel where one is obvious from `NetworkMeta`.
...
total per currency, plus any networks skipped (nothing past threshold) and any
that failed (verbatim error). Then present the drafts and the attachment(s).
