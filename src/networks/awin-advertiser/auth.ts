 ...
 * carried on `accountType` ("advertiser"/"publisher") on the live API; we read
 * `accountType` first and fall back to `type` for aliased payloads, then filter
 * on that in `listBrands` (see `./adapter.ts`).
 *
...
export interface AwinAdvAccountRaw {
  ...
  name?: string;
  /**
   * Live `GET /accounts` returns the account kind on `accountType`
   * ("advertiser"/"publisher"), not `type`. Read `accountType` first and fall
   * back to `type` for any aliased payload; reading `type` alone silently
   * filtered out every advertiser account (the `no-advertiser-accounts` bug).
   */
  accountType?: string;
  type?: string;
  /**
   * Live `/accounts` also returns a per-account `userRole`
   * (e.g. userViewer / userFullAccess / userOwner). Captured for future
   * tier/permission heuristics; not yet used to flip `apiEnabled`.
   */
  userRole?: string;
}
