/**
 * Validate the shipped FlexOffers network.json against the canonical schema.
 *
...
function loadManifest(): Record<string, unknown> {
  return JSON.parse(
    readFileSync(
      path.join(process.cwd(), 'src', 'networks', 'flexoffers', 'network.json'),
      'utf8',
    ),
  );
}

describe('FlexOffers network.json', () => {
  ...
  it('has the required FlexOffers-specific fields set correctly', () => {
    const raw = loadManifest();
    expect(raw.slug).toBe('flexoffers');
    expect(raw.name).toBe('FlexOffers');
    expect(raw.base_url).toBe('https://api.flexoffers.com');
    ...
    expect(raw.docs_url).toBe('https://www.flexoffers.com/publishers/web-service-api/');
    // Env vars must include the API key.
    expect(raw.env_vars).toContain('FLEXOFFERS_API_KEY');
  });
