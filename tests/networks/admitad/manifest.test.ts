/**
 * Validate the shipped Admitad network.json against the canonical schema.
 *
 * Mirror of `tests/networks/skimlinks/manifest.test.ts` — drift between the
 * manifest and the Zod schema is easy to miss because the manifest is loaded at
 * runtime by tooling rather than checked by TypeScript.
 */

import { describe, expect, it } from 'vitest';
import { readFileSync } from 'node:fs';
import path from 'node:path';

import { NetworkJsonSchema } from '../../../scripts/validate-network-json.js';

function loadManifest(): Record<string, unknown> {
  return JSON.parse(
    readFileSync(
      path.join(process.cwd(), 'src', 'networks', 'admitad', 'network.json'),
      'utf8',
    ),
  );
}

describe('Admitad network.json', () => {
  it('conforms to the canonical schema', () => {
    const raw = loadManifest();
    const r = NetworkJsonSchema.safeParse(raw);
    expect(r.success).toBe(true);
    if (!r.success) {
      throw new Error(JSON.stringify(r.error.issues, null, 2));
    }
  });

  it('has the required Admitad-specific fields set correctly', () => {
    const raw = loadManifest();
    expect(raw.slug).toBe('admitad');
    expect(raw.name).toBe('Admitad');
    expect(raw.base_url).toBe('https://api.admitad.com');
    expect(raw.auth_model).toBe('oauth2');
    expect(raw.side).toBe('publisher');
    expect(raw.credential_scope).toBe('single-brand');
    expect(raw.supports_brand_ops).toBe(false);
    expect(raw.setup_requires_approval).toBe(false);
    expect(raw.claim_status).toBe('experimental');
    expect(raw.adapter_version).toBe('0.1.1');
    expect(raw.last_verified).toBe('2026-06-04');
    expect(raw.docs_url).toBe('https://developers.admitad.com/');
  });

  it('declares the OAuth2 credential env vars', () => {
    const raw = loadManifest();
    const envVars = raw.env_vars as string[];
    expect(envVars).toContain('ADMITAD_CLIENT_ID');
    expect(envVars).toContain('ADMITAD_CLIENT_SECRET');
    expect(envVars).toContain('ADMITAD_WEBSITE_ID');
    // Every env var must satisfy the schema's [A-Z][A-Z0-9_]* shape.
    for (const v of envVars) {
      expect(v).toMatch(/^[A-Z][A-Z0-9_]*$/);
    }
  });

  it('lists the mandatory "built from public docs" limitation first', () => {
    const raw = loadManifest();
    const limits = raw.known_limitations as string[];
    expect(limits[0]).toBe(
      'Adapter built from public API documentation; not yet verified against a live account.',
    );
    expect(limits.some((s) => s.includes('not yet verified against a live account'))).toBe(true);
  });
});
