/**
 * Small shared helpers for the GrowSurf adapter. Kept out of `adapter.ts` so
...
/** Build a `config_error` NetworkError for the GrowSurf adapter. */
...
/**
 * Require an `AdapterCallContext` on advertiser-side operations. A GrowSurf API
 * key + campaign id scopes a single programme, so `networkBrandId` is currently
...
export function requireCtx(operation: string, ctx?: AdapterCallContext): AdapterCallContext {
  if (!ctx || !ctx.networkBrandId) {
    throw configErrorFor(
      operation,
      `GrowSurf ${operation} requires a brand context (networkBrandId).`,
      {
        hint:
