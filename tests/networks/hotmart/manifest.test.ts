/**
 * Validate the shipped Hotmart network.json against the canonical schema.
 *
...
function loadManifest(): Record<string, unknown> {
  return JSON.parse(
    readFileSync(
      path.join(process.cwd(), 'src', 'networks', 'hotmart', 'network.json'),
      'utf8',
    ),
  ) as Record<string, unknown>;
}

describe('Hotmart network.json', () => {
  ...
  it('has the required Hotmart-specific fields set correctly', () => {
    const raw = loadManifest();
    expect(raw.slug).toBe('hotmart');
    expect(raw.name).toBe('Hotmart');
    expect(raw.base_url).toBe('https://developers.hotmart.com/payments/api/v1');
    ...
    expect(raw.docs_url).toBe('https://developers.hotmart.com/docs/en/');
    // Env vars must include the two required OAuth credentials.
    expect(raw.env_vars).toContain('HOTMART_CLIENT_ID');
    expect(raw.env_vars).toContain('HOTMART_CLIENT_SECRET');
  });

  it('has the mandatory not-verified limitation as the FIRST entry', () => {
    const raw = loadManifest();
