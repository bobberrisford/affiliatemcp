/**
 * Small shared helpers for the Partnero adapter. Kept out of `adapter.ts` so
...
/** Build a `config_error` NetworkError for the Partnero adapter. */
...
/**
 * Require an `AdapterCallContext` on advertiser-side operations. A Partnero API
 * token scopes a single programme, so `networkBrandId` is currently
...
export function requireCtx(operation: string, ctx?: AdapterCallContext): AdapterCallContext {
  if (!ctx || !ctx.networkBrandId) {
    throw configErrorFor(
      operation,
      `Partnero ${operation} requires a brand context (networkBrandId).`,
      {
        hint:
