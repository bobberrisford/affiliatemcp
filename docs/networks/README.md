...
Every adapter under `src/networks/` has one matching setup guide here. The
generated network table in the root [`README.md`](../../README.md#networks) is
the current index of publisher-side and advertiser-side adapters; do not
maintain a second hand-written network list in this file.

## Source boundaries

- Setup requirements and capability claims come from the matching
  `src/networks/<slug>/network.json`.
- Credential instructions and dashboard paths belong in
  `docs/networks/<slug>.md`.
- Implementation evidence and research notes belong in
  [`../findings/`](../findings) and feed the generated
  [`REPORT.md`](../../REPORT.md).

Awin also has a [deeper API inventory](./awin/api-inventory.md), reflecting its
