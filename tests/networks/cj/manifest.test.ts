/**
 * Validate the shipped CJ network.json against the canonical schema.
 *
 * Mirror of `tests/networks/awin/manifest.test.ts` — drift between the
...
describe('CJ network.json', () => {
  it('conforms to the canonical schema', () => {
    const raw = JSON.parse(
      readFileSync(
        path.join(process.cwd(), 'src', 'networks', 'cj', 'network.json'),
        'utf8',
      ),
    );
